...
var SwipeView = (function() {
	var hasTouch = 'ontouchstart' in window, resizeEvent = 'onorientationchange' in window ? 'orientationchange' : 'resize', startEvent = hasTouch ? 'touchstart' : 'mousedown', moveEvent = hasTouch ? 'touchmove' : 'mousemove', endEvent = hasTouch ? 'touchend' : 'mouseup', cancelEvent = hasTouch ? 'touchcancel' : 'mouseup', SwipeView = function(el, options) {
	...
	SwipeView.prototype = {
		...
		goToPage : function(p) {
			...
			var prel = (this.options.numberOfPages < 3) ? this.options.numberOfPages : 3;
			for( i = 0; i < prel; i++) {
			...
			this.currentMasterPage = (this.page + 1) - Math.floor((this.page + 1) / prel) * prel;
		...
		__move : function(e) {
			...
			var point = hasTouch ? e.touches[0] : e, deltaX = point.pageX - this.pointX, deltaY = point.pageY - this.pointY, newX = this.x + deltaX, dist = Math.abs(point.pageX - this.startX);
		...
		__end : function(e) {
			...
			var point = hasTouch ? e.changedTouches[0] : e, dist = Math.abs(point.pageX - this.startX);
		...
		__checkPosition : function() {
			...
			if(this.x == newX) {
				this.__flip();
				// If we swiped all the way long to the next page (extremely rare but still)
			} else {
				this.__pos(newX);
